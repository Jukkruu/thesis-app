...
- **Email**: Resend via `src/lib/email.ts` — `sendStepEmail()` on every step advance, finance email at PROPOSAL step 3. All emails route to `outanagon2549@gmail.com` (test override); email body shows the intended recipient's name + email. Magic links are reusable and valid for 7 days (not one-time use).
...
### "ส่งกลับขั้นตอนก่อนหน้า" panel
A collapsible **send-back panel** is shown in `RoleSubmissionDetail` to **all roles** whenever there is an active (PENDING) step and submission status is not `COMPLETED` or `CANCELLED`. It shows which role the workflow will be sent back to, accepts optional notes, and calls `action: "reject"`.
