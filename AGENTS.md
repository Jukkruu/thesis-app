...
- **Email**: SMTP via nodemailer in `src/lib/email.ts` (shared `sendMail()` helper) — `sendStepEmail()` on every step advance, `sendFinanceEmail()` at PROPOSAL step 3 and THESIS step 6 (called directly, not via HTTP). Emails go to real recipients. Sender: Office365/generic SMTP when `SMTP_USER`/`SMTP_PASS` are set (default host smtp.office365.com:587), else Gmail via `GMAIL_USER`/`GMAIL_APP_PASSWORD` (~500 emails/day limit). Emails contain a **plain-text magic-link URL** (no button) plus a password-login fallback line. Magic links are valid 48h and **NOT consumed on first use** (deleted only at expiry) — Office365 SafeLinks prefetches emailed URLs, which used to kill one-time tokens before recipients clicked. Token creation lives in `createMagicLink()` in `email.ts`.
