...
export function WorkflowTimeline({
  ...
  // Split into visible (done + current) and future (not yet reached)
  const visibleSteps = steps.filter(
    (s) => s.status === "APPROVED" || s.status === "REJECTED" || s.stepOrder === currentOrder
  );
  const futureSteps = steps.filter(
    (s) => s.status === "PENDING" && s.stepOrder !== currentOrder
  );

  function renderStep(step: MockWorkflowStep, isFuture = false) {
  ...
  return (
    <ol className="relative border-l-2 border-gray-100 ml-3 space-y-0">
      ...
      {/* Collapsible future steps */}
      {futureSteps.length > 0 && (
        <>
          <li className="ml-6 pb-4">
            <span className="absolute -left-3.5 flex items-center justify-center w-7 h-7 rounded-full ring-4 ring-gray-50 bg-white">
              <Circle className="w-5 h-5 text-gray-200" />
            </span>
            <button
              type="button"
              onClick={() => setShowFuture((v) => !v)}
              className="flex items-center gap-2 text-sm text-gray-400 hover:text-gray-600 transition py-1"
            >
              {showFuture ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              {showFuture ? "ซ่อนขั้นตอนที่เหลือ" : `ขั้นตอนที่เหลืออีก ${futureSteps.length} ขั้น`}
            </button>
          </li>

          {showFuture && futureSteps.map((step) => renderStep(step, true))}
        </>
      )}
    </ol>
  );
